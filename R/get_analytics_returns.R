...
## three packages for financial analytics
# library(PortfolioAnalytics)
# library(PerformanceAnalytics)
# library(quantmod)


get.all.annual.returns <- function(){
  ...
  if(!(rlang::is_empty(list.files(paste0(path.data.processed.analytics.pricequantitypanel))))){
    ...
    for(i in 1:length(list.dfs)){
      ...
      df.temp <- get.annual.returns(list.dfs[[i]])
      
      df <- merge(df,df.temp,by="year",all.x=T,all.y=T)
    ...
    filename.annual.returns <- paste0("all_annual_returns_from_",min.year,"_to_",max.year,".csv")
    
    ## store as csv
    data.table::fwrite(df,paste0(path.data.processed.analytics.returns,filename.annual.returns))
  
  } else (return("No price-quanity panel to calculate annual returns.")) ## end of if else statement

} ## end of function get.all.annual.returns



get.annual.returns <- function(df){
...
} ## end of function get.annual.returns
...
if(!(rlang::is_empty(list.files(paste0(path.data.processed.analytics.returns),pattern="^all_annual_returns_from_")))){
  
  ## file name for annual returns
  filename.annual.returns <- list.files(paste0(path.data.processed.analytics.returns),pattern="^all_annual_returns_from_")
  
  ## choose annual returns with most recent year and minimum year
  filename.annual.returns <- filename.annual.returns[grepl(max(as.numeric(stringr::str_match(filename.annual.returns, "to_(.*?).csv")[,2])),filename.annual.returns)]
  filename.annual.returns <- filename.annual.returns[grepl(min(as.numeric(stringr::str_match(filename.annual.returns, "from_(.*?)_to_")[,2])),filename.annual.returns)]
  
  ## load annual returns
  df.annual.returns <- data.table::fread(paste0(path.data.processed.analytics.returns,filename.annual.returns))
  ...
  df.annual.returns[nrow(df.annual.returns),"Year"] <- min(df.annual.returns$Year,na.rm=T)-1
  ...
  for(i in 2:length(df.annual.returns)){
    ...
    df.temp <- data.frame(ticker,annual.returns.1y,annual.returns.3y,annual.returns.5y,annual.returns.10y,annual.returns.max)
  ...
  if(length(df.annual.returns) > 3){
    df.annual.returns[,names(df.annual.returns)!="Year"] <- apply(df.annual.returns[,names(df.annual.returns)!="Year"],2,formatC,digits=2,format="f")
  } else if(length(df.annual.returns)==2){df.annual.returns[,2] <- formatC(df.annual.returns[,2],digits=2,format="f")}
