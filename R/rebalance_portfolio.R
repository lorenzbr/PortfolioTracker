...
#' @param df.target.shares A data frame containing a column for \code{isin}
...
#' @param exclude.from.rebalancing A single character or vector of strings.
...
rebalance_portfolio <- function(df, df.target.shares, money.to.invest, step.size = 500,
  ...
  for (i in 1:nb.iter) {
    ...
    is_isin_with_max_deviation <- df$isin == isin.with.max.deviation

    ## invest in ISIN with max deviation
    df$new_value[is_isin_with_max_deviation] <- df$new_value[is_isin_with_max_deviation] + step.size
    df$value_to_invest[is_isin_with_max_deviation] <- df$value_to_invest[is_isin_with_max_deviation] + step.size
...
write_default_target_shares <- function(df.target.shares, path) {
  ...
  if (is.data.frame(df.target.shares)) {
    ...
    if (all(c("isin", "target_share") %in% names(df.target.shares)))
      data.table::fwrite(df.target.shares,
