...
write_current_portfolio <- function(path) {
  ...
  if ( !rlang::is_empty(list.files(path.pricequantity.panel)) ) {
    ...
    if ( transaction.history.exists && isin.ticker.exists ) {
      ...
      df.ticker.investmentnames <- unique(df.transaction.history[, c("isin", "name", "transaction_date")])

      ## Use name from most recent transaction
      df.ticker.investmentnames$date <- as.Date(df.ticker.investmentnames$transaction_date, "%d-%m-%Y")
