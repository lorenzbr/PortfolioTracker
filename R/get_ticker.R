...
#' @param file A single character string. Name of ISIN-ticker csv
#' file (default: isin_ticker.csv).
#'
#' @export
init_isin_ticker <- function(path, file = "isin_ticker.csv") {
  ...
  if (!file.exists(file.path(path, file))) {
    ...
    df_isin_ticker_init <- data.frame(
      matrix(NA, nrow = 0, ncol = length(col_names),
...
#'
#' @usage update_ticker_isin(isins, path_tickers, file_tickers,
#'                           external_search = TRUE)
#' @param isins A single character or vector of strings. ISINs.
#' @param path_tickers A single character string. Folder where ISIN-ticker
#' table is stored.
  ...
  if (length(new_isins) > 0) {
    ...
    for (i in 1:length(new_isins)) {
      ...
      try({
        ...
        ## WiP, better to include function argument with whatever
        ## data is collected
        df_isin_ticker_list <- data.table::fread(
          system.file("extdata", "isin_ticker_name_list_xetra_june2017.csv",
                      package = "PortfolioTracker"))
        df_isin_ticker_list <- as.data.frame(df_isin_ticker_list)
        ticker <- df_isin_ticker_list$ticker[df_isin_ticker_list$isin == isin]
...
#' @param preferred_exchange A single character string. Stock
#' exchange (e.g., Xetra)
#'
#' @return A single character string containing the ticker for the given ISIN.
#'
#' @export
get_ticker_from_investing <- function(isin, preferred_exchange = "") {
  ...
  if (length(html_output_text_selected) == 0) {
    ...
    html_output_text_selected <- html_output_text[sample(length(html_output_text),
                                                         1)]
  }
...
#'
#' @usage add_ticker_manually(df_isin_ticker_new, path.tickers,
#'                            file.ticker = "isin_ticker.csv")
#' @param df_isin_ticker_new A data frame containing a column for \emph{isin}
#' and \emph{ticker}.
#' @param path.tickers A single character string. Folder where ISIN-ticker
#' table is stored.
#' @param file.ticker A single character string. Name of ISIN-ticker csv file
#' (default: isin_ticker.csv).
#'
#' @export
add_ticker_manually <- function(df_isin_ticker_new, path.tickers,
  ...
  ## Identify all ISINs in transaction data and check whether
  ## the corresponding ticker is in the table already
  new_isins <- isins[!(isins %in% unique(df_isin_ticker$isin))]
  ...
  if (length(new_isins) > 0) {
    ...
    for (i in 1:length(new_isins)) {
      ...
      try({
        ...
        ## If no regex is needed, grepl is faster with fixed = TRUE
        df_isin_ticker_add <- df_isin_ticker_new[grepl(isin,
                                                       df_isin_ticker_new$isin,
                                                       fixed = TRUE), ]
